...
### 9.18 Video Player のコールバックだけではロード世代を識別できない
- `OnVideoError` などの Video Player コールバックには、どの `LoadURL` 呼び出しに対応するかを示す世代番号がない。URL を短時間に差し替えた場合、コールバックだけを根拠に現在のロードを即座に失敗状態へ遷移させると、新しいロードの成功を妨げる可能性がある。
