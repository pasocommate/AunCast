...
- AVPro 出力テクスチャにはプラットフォーム差がある:
  - PC (D3D11): 上下反転 + 非 sRGB（ガンマ空間）→ シェーダー内の固定 `pow(color, 2.2)` による手動ガンマ補正と、`_MainTex_ST` 負スケール検出（`GetVideoFlipY()`）による反転処理で対応（現行実装）
  - Android (Quest): 反転はランタイム自動検出がそのまま追従する。ガンマも **PC と同じ固定 `pow(2.2)` で正しく表示される**ことを実機確認済み（2026-07-12）
  - 当初は「Android の AVPro 出力は sRGB 変換済み」と推測し、`AunCastScreen` / `AunCastUiScreen` が映像表示中のみ `_Gamma=1.0` を設定して補正を無効化する対処を入れたが、実機で映像が白飛びしたため撤去した。プラットフォーム差を前提にガンマを分岐しないこと（詳細は [Implementation-Patterns.md](Implementation-Patterns.md) §14）
...
AunCastPlaybackMonitor.OnPlayerLeft(VRCPlayerApi player):
  ...
  foreach slot i in 0..MAX_PLAYERS:
    ...
    clear playbackActive[i], connectingActive[i], errorActive[i]
...
- **停止(Stop All)時のリングバッファ・クリア**: 委譲先 `AudioOutputTunnel` は入力を `AudioSource.GetOutputData`（volume 反映済み）で読み取り、リングバッファ経由で出力する。停止すると直前の PCM がバッファに残り、短いループ音が鳴り続ける。これを防ぐため、`AunCastAudioOutputTunnel` は `controller`（`AunCastDualPlayerController`）を参照し、`GetLocalState()` が `STATE_IDLE`（＝停止）を返す間は `input` を無音側（volume 0 の Standby AudioSource）へ差し替える。volume 0 の `GetOutputData` は全ゼロを返すため、無音を流し込むことでバッファが無音で上書きされ、ループ音が消える。`controller` 未設定時はこのクリアは行われず、従来通り可聴側追従のみになる。`controller` は「参照関係を再配線」で自動配線される
