...
- **自動移行（実装済み）**: 変換候補の「トンネル移行」ボタンは、旧 `AudioOutputTunnel` を削除せず温存し、同じ GameObject に `AunCastAudioOutputTunnel` を追加して `targetTunnel` に設定する。旧 `input` の AudioSource は同じ階層の直後に複製し、オリジナルを PlayerA、複製を PlayerB の `AunCastSpeaker` として設定して `inputA` / `inputB` に割り当てる。委譲先になった旧 `AudioOutputTunnel` は変換候補一覧に再列挙しない。
...
- 互換アダプタの A/B 入力には **旧トンネルの入力用 AudioSource を A/B 用に複製・変換した AunCastSpeaker**を使う。再配線は既存の `inputA` / `inputB` を上書きせず、未設定の場合のみ PlayerA/B の既存 `AunCastSpeaker` で補完する。`targetTunnel` も既存値を尊重し、未設定の場合のみ同一 GameObject 上の `AudioOutputTunnel` で補完する。
