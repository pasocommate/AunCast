...
| ⑩ **ヘルプ表示** | ポイントしたUI部品の説明欄。押すと日本語 / 英語を切替 |

<!--@rv タイムラインログ、スタッフ操作ロックボタンの説明も必要。Resync、Rebootボタンも挙げて（ヘルプ表示は既にここに挙がっているが、それも加えて）観客ビューのものと同じである旨を記載。画像への番号の再埋め込みが必要だが、フォントや色が変わらないようにしたい。どうしても変わってしまう場合はpanel-viewer.mdの同様の校正指示と併せて再生成 -->
<!--@rv タイムラインログは負荷が高い旨の警告表示をsettings.mdからここに移設 -->
