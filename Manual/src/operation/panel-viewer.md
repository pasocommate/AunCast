...
このページでは、手元パネル（観客ビュー）の使い方と、各表示の見方を説明します。スタッフも、自分の端末の再同期や音量調整では、この観客ビューの操作を使用します。手元パネルの呼び出し方は[壁パネルと手元パネル](panels.md)を、配信URLの変更や全員一斉操作などスタッフ専用の操作は[手元パネル：スタッフビュー](panel-staff.md)を参照してください。事前に[再同期の仕組み](../concepts/resync.md)と[同時接続上限の管理](../concepts/connection-limit.md)を読んでおくと、各操作の意味を理解しやすくなります。
...
!!! warning "オフにしても、他の自動復旧は停止しません"
    オフにするのは「**無音を契機とする自動再同期のみ**」です。再生停止時の自動再同期や、手動の Resync ボタンは、オフにしても従来どおり有効です。
