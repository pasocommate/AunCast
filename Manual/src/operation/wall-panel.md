...
壁パネルは、ワールドに固定設置された操作パネルです。**近づくと表示が自動で切り替わります。**
...
!!! info "スタッフ用の入口もここにあります"
    右下の鍵アイコンから、スタッフ用の暗証番号を入力する画面に切り替えられます。詳細は[スタッフ画面を解錠する](panel-staff.md#unlock)を参照してください。

    鍵アイコンは、壁パネルごとの設定（`WallControlPanel` の **Disable Passcode View Switch Button**）で非表示にできます。暗証番号を空欄にして無効化している場合は、入力しても解錠されないため、あわせて鍵アイコンも非表示にすることを推奨します。

---

## 手元パネルを呼び出す

手元パネルは、いつでも手元に表示できるメニューです。呼び出し方法は、壁パネルの **Spawn Gesture** で選択します。この選択状態はローカル設定としてワールドごとに保存され、他の観客には影響しません。
...
!!! tip "押し続ける方式ではHUDゲージが表示されます"
    ...
    <!--@ph スクリーンショット: 長押しジェスチャー中に視界へ表示されるHUDゲージの実表示（VR視点またはデスクトップ視点）。ゲージが途中まで進んだ状態が望ましい。-->
    ![HUDゲージの表示例](../assets/hud-gesture-gauge.png){ width="300" }
