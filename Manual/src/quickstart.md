...
1. 壁パネルの鍵アイコンを押し、**暗証番号** でスタッフビューを解錠します。**スタッフ許可ユーザー名** に登録されている場合、この手順は不要です。
...
3. 手元パネルをスタッフビューへ切り替えます。ジェスチャーをもう一度行うか、パネル右上の **⇔ボタン** を押します。
...
3. 再生が安定せず復旧しない・Resync の順番が回ってこない場合は、**Reboot ボタン**（⚡）を使用します。音声・映像が一時的に途切れます（[→ Reboot ボタン](operation/panel-viewer.md#reboot-button)）。
