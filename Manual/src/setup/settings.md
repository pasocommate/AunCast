...
| 低遅延モード | オン | AVProの低遅延モード（PC専用。Questでは無視されます） |
| クロスフェード時間 [秒] | 0.1 | 切替時の音声クロスフェード長 |
| 音声のみ配信フォールバック | オン | 映像が取得できなくても音声のみ配信として切替を完了 |
| 音声のみ判定待ち [秒] | 3.0 | 音声のみ配信として扱うまで、予備系統の映像テクスチャ到着を待つ時間 |
| 音声のみ前進判定 [秒] | 0.5 | 音声のみ配信として扱うために必要な、予備系統の再生時刻の前進量 |
| 停止中のスクリーン画像 | Blank-AunCast.jpg | 停止時に表示する固定画像 |

!!! warning "「停止中のスクリーン画像」のインポート設定"
    「停止中のスクリーン画像」に独自の画像を指定する場合は、既定画像（Blank-AunCast.jpg）を参考に、テクスチャのインポート設定を以下のようにしてください。

    - **sRGB (Color Texture)** をオフに設定（AVProの色処理に合わせるため）
    - **Non-Power of 2** を **None** に設定（既定のままだと２の累乗サイズへ丸められ、手元パネル表示時の縦横比が崩れます）
...
!!! warning "タイムラインログは高負荷"
    **有効にすると負荷がかかります。** 通常運用ではオフのままにしてください。スタッフビューのトグルからも切り替えられます。運用中の切り替え方法と詳しい注意は[手元パネル：スタッフビュー](../operation/panel-staff.md#timeline-logging)を参照してください。
