...
| **同時Resync上限 [人]** | [同時接続上限の管理](../concepts/connection-limit.md#limit-params)の式で上限を見積もり、余裕を見た値（初期値の **10** が出発点） |
...
| 表示名 | 既定 | 意味 |
|---|---:|---|
| 初期音量 | 0.6 | 各ユーザーの起動時ローカル音量（０〜１） |
| デフォルト配信URL | 空欄 | Next URL欄の初期値（最初のJoin時の自動再生にも使用） |
| 最初のJoinで自動再生 | オフ | 最初のユーザー入室時にデフォルトURLを自動再生 |
| VR呼び出しジェスチャー初期値 | 右スティック上長押し | HUD（手元パネル）呼び出し方法の初期有効設定 |
| デスクトップ呼び出しジェスチャー初期値 | Tabダブルタップ | HUD（手元パネル）呼び出し方法の初期有効設定 |
...
| パネル自動閉じ距離 [m] | ３ | 手元パネルから離れると自動で閉じる距離（０で無効） |
| パネル視界外閉じ [秒] | 20 | 手元パネルが視界外に出てから閉じるまでの秒数（０で無効） |
