...
- **同時接続上限** … 契約している **配信プランの同時接続上限** を入力します（例: 100）。
...
!!! warning "自動再生は原則オフ推奨・運用上の注意"
    **「最初のJoinで自動再生」は原則として使用しない**ことを推奨します。理由と、CDNの選定・ビットレート・URLの取り扱いなどの運用上の注意は、[配信・運用上の注意](../operation/streaming.md)を必ず確認してください。
