...
具体的な調整方法と判断の目安は[モニタリングと上限調整](../guide/monitoring.md#limits)を参照してください。

---

## 「全員一斉」でも上限を超えない {#staggering}

スタッフが **全員一斉の再同期**（[Resync All](../guide/operations.md#global-resync)）を実行しても、上限を超える心配はありません。一斉といっても全員が同時に接続し直すわけではなく、**通常の再同期と同じ順番待ちの列に、全員がまとめて並ぶ** だけだからです。
...
!!! tip "配信側の注意もあわせて"
    CDNの選定、ビットレート、URLの取り扱い、自動再生など、**配信側の運用上の注意**は [配信・運用上の注意](../guide/streaming.md) にまとめています。接続の安定にはこちらも重要です。
