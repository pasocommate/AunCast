...
ライブ配信の音声・映像は、配信者から **配信サーバー**（[VRCDN](https://vrcdn.live/) や [TopazChat](https://github.com/TopazChat/TopazChat) など）を通じて各観客へ届きますが、**「同時に受け付けられる接続数」の上限** があります。本マニュアルでは例として **「100接続」** のサービスを前提に説明します。
...
AunCast は **順番を管理する仕組み**（コーディネーター）を備えており、再同期を **予約制（順番待ち）** にしています。処理の流れは以下のとおりです。
...
接続管理に関わる数値は２つあり、いずれも **スタッフがワールド内から、その場で変更** できます（変更は全員に反映されます）。スタッフビューでは以下のラベルで表示されます。

| **表示項目** | Connection≤ | Concurrent≤ |
|---|---|---|
| **マニュアルでの記載** | 「同時接続上限」 | 「同時Resync上限」 |
| **意味** | 配信サーバーの接続数上限 | 同時に再同期を許可する最大人数 |
| **設定の目安** | 配信サービスの上限 | 下記の式の値から余裕を見た数値 |

**Concurrent≤** に設定できる上限の目安は、以下の式で見積もることができます。

`Concurrent≤` の目安 ＝ `Connection≤` − 想定される最大インスタンス人数

例：上限100接続・最大80名収容 → 100 − 80 ＝ **20 が上限の目安**。

実際には、この目安より十分に小さめの値（初期値の **10** 程度）から始めてください。大きすぎると観客ごとの遅延にバラつきが生じ、再同期の失敗も増えます。一方、小さすぎると順番待ちが滞留します。本番中の調整方法と判断の目安は[モニタリングと上限調整](../operation/monitoring.md#limits)を参照してください。
...
!!! tip "配信側の注意もあわせて"
    CDNの選定、ビットレート、URLの取り扱いなど、**配信側の運用上の注意**は [配信・運用上の注意](../operation/streaming.md) にまとめています。接続の安定にはこちらも重要です。
