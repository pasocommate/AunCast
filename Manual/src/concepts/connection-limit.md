...
ライブ配信の音声・映像は、配信者から **配信サーバー**（VRCDN や TopazChat など）を通じて各観客へ届きますが、**「同時に受け付けられる接続数」の上限** があります。本マニュアルでは例として **「100接続」** のプランを前提に説明します。
...
| 項目 | **Connections** | **Concurrent** |
|---|---|---|
| スタッフ画面の表示 | Connections（同時接続上限） | Concurrent（同時Resync上限） |
| 意味 | 配信サーバーが受け付ける接続数上限 | 同時に再同期してよい人数 |
| 設定の目安 | **契約配信プランの上限** 以下（例: 100） | 空き接続から余裕を見て **5〜15** 程度 |
...
あとは「同時にResyncしてよい人数」の枠ごとに、順次処理されていきます。
...
- 上限は **Connections（同時接続上限）** と **Concurrent（同時Resync上限）** の２つがあり、**スタッフがワールド内で調整** できる。
