...
自動Resyncは、各観客のローカル環境で判断されます。主な判定は次の３つです。

| 判定 | 内容 |
|---|---|
| 停止検知 | 再生位置が進まない状況が一定時間続く |
| ドリフト検知 | 配信元との再生位置のズレがしきい値を超える |
| 無音検知 | 音声が出ていない状況が一定時間続く |

無音検知は、曲間や静かな演出で不要に働く場合があります。そのため、観客は手元パネルの **Silence Resync** で、無音を契機とする自動Resyncだけをオフにできます。
...
予備系統で接続できない場合、AunCast は状況に応じて処理を分けます。

- **現用系統がまだ再生できている場合**：現用系統を維持し、時間を置いて再試行します。
- **現用系統も予備系統も再生できない場合**：間隔を徐々に広げながら、自動で再接続を繰り返します。

このとき、手元パネルには「再接続待機中（あと○秒）」などの再生ステータス表示が出ます。失敗が続く場合は、スタッフ画面の状態表示にも反映されます。
