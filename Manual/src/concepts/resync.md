...
観客は手元パネルの **Mode** を Manual にすると、停止・ドリフト・無音・再生エラー等を契機とするすべての自動Resyncを停止できます。スタッフは **Force Mode** で全観客のモードを Auto または Manual に上書き・強制できます（→ [Force Mode](../operation/panel-staff.md#force-mode)）。Resync / Reboot ボタン押下時の動作と、スタッフによる操作は、モードに関わらずいつでも有効です。
