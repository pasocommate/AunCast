...
namespace PasocomMate.AunCast
{
    ...
    public class UserStatusPanel : UdonSharpBehaviour
    {
        ...
        // グリップボタンの生押下状態（#23 右スティックジェスチャー抑制用）
        ...
        private const float BG_DISSOLVE_THRESHOLD_DEFAULT = 0f;
        ...
        private void Start()
        {
            ...
            {
                Material sourceMaterial = backgroundImage.material;
                if (sourceMaterial != null)
                {
#if COMPILER_UDONSHARP
                    // Udon は非 GameObject 型の Instantiate をサポートしない。
                    _bgMaterial = sourceMaterial;
#else
                    // 共有マテリアルを直接変更すると Play 後にアセットへ値が残るため、実行時インスタンスを使う。
                    _bgMaterial = (Material)UnityEngine.Object.Instantiate(sourceMaterial);
                    backgroundImage.material = _bgMaterial;
#endif
                    _bgMaterial.SetFloat("_DissolveThreshold", BG_DISSOLVE_THRESHOLD_DEFAULT);
                }
            }
        ...
        private void RestoreBackgroundDissolveThreshold()
        {
        ...
        {
            // 他ボタン押下中または右手が視界外の場合はジェスチャーをリセットして抑制 (#23, #24)
            if (_isVRChatMenuOpen || _vrLookVertical < vrRightStickUpThreshold
                || _vrLeftUsePressed || _vrRightUsePressed || _vrLeftGrabPressed || _vrRightGrabPressed
                || !IsHandInView(HandType.RIGHT, local))
            {
        ...
        private void HandleDoubleTriggerEdge(bool isLeft)
        {
            ...
            // 操作する手が視界外の場合は発動を抑制 (#24)
            if (!IsHandInView(isLeft ? HandType.LEFT : HandType.RIGHT, local)) return;
        ...
        /// <summary>指定した手がヘッドの前方半球内にあるか判定する。視界外ジェスチャー抑制 (#24) に使用。</summary>
        private bool IsHandInView(HandType hand, VRCPlayerApi local)
        {
            VRCPlayerApi.TrackingData head = local.GetTrackingData(VRCPlayerApi.TrackingDataType.Head);
